...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await query(
      `DELETE FROM crm_session_players WHERE session_id = $1`,
      [id]
    );
    ...
    await syncSessionToGoogleCalendarsSafe(id, 'session players update');

    return jsonResponse({ success: true });
  } catch (error) {
    console.error('Error updating session players:', error);
    return errorResponse('Failed to update session players');
  }
}
