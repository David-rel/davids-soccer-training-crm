...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return jsonResponse(await getSessionExtras('session', id));
  } catch (error) {
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await setSessionExtras('session', id, parseExtraPlayerIds(body.extra_player_ids));
    if (!result.ok) return errorResponse(result.error, result.status);

    // Extra parents ride on the calendar invite, so the event needs a re-sync.
    await syncSessionToGoogleCalendarsSafe(id, 'session extras update');
